...
export const Step1: React.FC<any> = ({ data }) => {
  ...
  return (
    <S.FormContent>
      ...
      >
        {data && (
          <DatePicker defaultValue={moment(data['result1'][0].date_reg_unp, 'DD-MM-YYYY')} format={'DD-MM-YYYY'} />
        )}
      </BaseButtonsForm.Item>
          ...
          {data ? (
            <List
              ...
              renderItem={(item: { name_build: string; id_specif: string; id_unit_41: string }) => (
                <List.Item>{item.name_build}</List.Item>
              )}
            />
          ) : (
            <p></p>
      ...
      <BaseButtonsForm name="dobavit_zdan" isFieldsChanged={isFieldsChanged} onFinish={onFinish} autoComplete="off">
            ...
            <>
              {fields.map((field) => (
                <Row key={field.key} style={{ marginTop: '35px' }}>
                  <Col span={6}>
                    <BaseButtonsForm.Item {...field} name={[field.name, 'last']} hasFeedback>
                      <S.Wrapper>
                        <Input />
      ...
      <BaseButtonsForm name="dobavit_sooryg" isFieldsChanged={isFieldsChanged} onFinish={onFinish} autoComplete="off">
        <BaseButtonsForm.List name="users">
          {(fields, { add, remove }) => (
            <>
              {fields.map((field) => (
