...
import { getCpuUsage, getMemorySize, getMemoryStatus } from '@app/api/serverStats.api';
...
export const ServerStatistics: React.FC = () => {
  ...
  //   useEffect(() => {
  //     getMemorySize().then((value) => {
  //       setMemorySize(value);
  //     });
  //   }, []);

  //   useEffect(() => {
  //     getMemoryStatus().then((value) => {
  ...
  const dbConfig = {
    percent:
      memorySize !== null
        ? memorySize.length === 2
          ? Number(memorySize[0].percentage.slice(memorySize[0].percentage.length - 2, 1)) / 100
          : 0.5
        : 0.5,
    range: {
      ticks: [0, 1 / 3, 2 / 3, 1],
  ...
  const appConfig = {
    percent:
      memorySize !== null
        ? memorySize.length === 2
          ? Number(memorySize[1].percentage.slice(memorySize[1].percentage.length - 2, 1)) / 100
          : 0.5
        : 0.5,
    range: {
      ticks: [0, 1 / 3, 2 / 3, 1],
        ...
        <Col className="gutter-row" span={8}>
          <Badge.Ribbon text="Активен" color="green">
            <Card title="Диск 1" bordered={false}>
              <p>
                Статус: <Badge status="success" text="Активен" />
              </p>
            </Card>
          </Badge.Ribbon>
        </Col>
