    ...
    const rowSelection: TableRowSelection<SSubjObj> = {
        ...
        onSelect: (record, selected, selectedRows) => {
           if(selected){
            ...
            prevState.filter((prevItem) => prevItem !== record.idObj));
           }
        },
      ...
      const rowSelectionSpecif: TableRowSelection<SSubjObjSpecif> = {
        ...
        onSelect: (record, selected, selectedRows) => {
            if(selected){
                ...
                setSelectedObj((prevState) =>
                prevState.filter((prevItem) => prevItem !== record.idSpecif));
               }
        },
        onSelectAll: (selected, selectedRows, changeRows) => {
          ...
          if(selected){
            ...
            setSelectedObj((prevState) =>
            prevState.filter((prevItem) => prevItem !== selectedRows[0].idSubjObj));
           }
        },
      };
    ...
    const expandedRowRender = (record:any) => {
        ...
        console.log(objSpecifMap);
            objSpecifMap.get(record.idObj)?.forEach((element)=>{
                if(element.idUnit_41 == 4000)
                    data1.push(element);
                else if(element.idUnit_41 == 4001)
                    data2.push(element);
                else if(element.idUnit_41 == 4003)
                    data3.push(element);
            })
            console.log(data1);
                return (
            <Collapse>
                <Panel header="Здания" key="1">
                   <Table dataSource={data1} columns={columns} rowSelection={{ ...rowSelectionSpecif }} rowKey={(record) => record.idSpecif}>

                   </Table>
                </Panel>
                <Panel header="Сооружения" key="2">
                    <Table
                        dataSource={data2} columns={columns} rowSelection={{ ...rowSelectionSpecif }} rowKey={(record) => record.idSpecif}
                    >
                   </Table>
                </Panel>
                <Panel header="Наружные установки" key="3">
                    <Table
                        dataSource={data3} columns={columns} rowSelection={{ ...rowSelectionSpecif }} rowKey={(record) => record.idSpecif}
                    >
                   </Table>
                </Panel>
            </Collapse>
    ...
    return (
        <>
            <Table rowKey={(record) => record.idObj}
                columns={columns}
                expandable={{expandedRowRender, onExpand(expanded, record) {
