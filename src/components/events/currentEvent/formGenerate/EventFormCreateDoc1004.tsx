import { generateSpravkaProverki } from '@app/api/doc.api';
...
const EventFormCreateDoc1004: React.FC<EventFormCreateDoc> = ({ event, toggleModal }) => {
  ...
  const onFinishCreateDocUved = (values: IFormReport) => {
    ...
    console.log(field);
    if (idEventOrder) {
      initGenerateDocGetIdList(field, idEventOrder, 1004).then((idList) => {
        generateSpravkaProverki({
          id_list: idList,
...
export default EventFormCreateDoc1004;
