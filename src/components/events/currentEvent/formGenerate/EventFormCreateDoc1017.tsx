import { generatePostanObOsvobjdeniiOtAdminOtvetstvenosti } from '@app/api/doc.api';
...
const EventFormCreateDoc1017: React.FC<EventFormCreateDoc> = ({ event, toggleModal }) => {
  ...
  const onFinishCreateDocUved = (values: IFormReport) => {
    ...
    console.log(field);
    if (idEventOrder) {
      initGenerateDocGetIdList(field, idEventOrder, 1017).then((idList) => {
        const newEventForm = { ...eventForm.getFieldsValue() };
        updateEventOrder(idEventOrder, newEventForm).then(() => {
          generatePostanObOsvobjdeniiOtAdminOtvetstvenosti({
            id_list: idList,
...
export default EventFormCreateDoc1017;
