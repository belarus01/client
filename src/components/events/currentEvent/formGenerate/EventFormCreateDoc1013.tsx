import { generatePostanONalojAdminVziskaniyaPoSovokupnosti } from '@app/api/doc.api';
...
const EventFormCreateDoc1013: React.FC<EventFormCreateDoc> = ({ event, toggleModal }) => {
  ...
  const onFinishCreateDocUved = (values: IFormReport) => {
    ...
    console.log(field);
    if (idEventOrder) {
      initGenerateDocGetIdList(field, idEventOrder, 1013).then((idList) => {
        const newEventForm = { ...eventForm.getFieldsValue() };
        updateEventOrder(idEventOrder, newEventForm).then(() => {
          generatePostanONalojAdminVziskaniyaPoSovokupnosti({
            id_list: idList,
...
export default EventFormCreateDoc1013;
