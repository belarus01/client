import { generateTrebOPriostanovleniiProizvodstva } from '@app/api/doc.api';
...
const EventFormCreateDoc1015: React.FC<EventFormCreateDoc> = ({ event, toggleModal }) => {
  ...
  const onFinishCreateDocUved = (values: IFormReport) => {
    ...
    console.log(field);
    if (idEventOrder) {
      initGenerateDocGetIdList(field, idEventOrder, 1015).then((idList) => {
        generateTrebOPriostanovleniiProizvodstva({
          id_list: idList,
...
export default EventFormCreateDoc1015;
