import { generatePostanONalojAdminVziskaniyaCh3St10_3 } from '@app/api/doc.api';
...
const EventFormCreateDoc1016: React.FC<EventFormCreateDoc> = ({ event, toggleModal }) => {
  ...
  const onFinishCreateDocUved = (values: IFormReport) => {
    ...
    console.log(field);
    if (idEventOrder) {
      initGenerateDocGetIdList(field, idEventOrder, 1016).then((idList) => {
        const newEventForm = { ...eventForm.getFieldsValue() };
        updateEventOrder(idEventOrder, newEventForm).then(() => {
          generatePostanONalojAdminVziskaniyaCh3St10_3({
            id_list: idList,
...
export default EventFormCreateDoc1016;
